...
- `pricing.csv` inherits #27 的動態 threshold rows，並新增 Grok 4.5 定價；不為已下架 Grok Build 0.1 建立估算費率。
...
- `pricing.csv` 保留 #27 的 Gemini/Claude 200K 門檻與 Grok 4.5 定價；歷史 `grok-build-0.1` 不會回退套用 Grok 4.5 費率。
...
- [x] 驗證歷史 Grok Build 0.1 識別碼不會套用 Grok 4.5、未知模型、負數/進位 timestamp 與 EOF timeline 行為。
...
- PR #30：補 `costUsdTicks`/flags、保留 `modelUsage` 模型歸因、區分歷史 `grok-build-0.1` 且不建立現行估算費率、未知模型不可猜成 Grok 4.5、修正 timestamp normalization 與 EOF incomplete turn 時間。
